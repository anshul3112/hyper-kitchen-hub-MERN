...
function OrdersView() {
  ...
  const load = useCallback(async (cursor?: string, prevCursor?: string) => {
    ...
    setError("");
    try {
      const result = await fetchOutletOrderHistory({ cursor, prevCursor, perPage: 10, status, startDate, endDate });
...
function HourlyView() {
  ...
  const handleViewHourDetails = async (point: HourlyPoint) => {
    ...
    try {
      const result = await fetchOutletHourlyOrders({
        date,
        hour: point.hour,
      });
