...
function OrdersView({ outlets }: { outlets: Outlet[] }) {
  ...
  const load = useCallback(
    async (cursor?: string, prevCursor?: string) => {
      setLoading(true);
      setError("");
      try {
        const result = await fetchTenantOrderHistory({ cursor, prevCursor, perPage: 10, outletId, status, startDate, endDate });
    ...
    [outletId, status, startDate, endDate]
  );
  ...
  return (
    <div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-4 mb-5 flex flex-wrap gap-3 items-end">
        ...
        <button
          onClick={() => { setOutletId(""); setStatus(""); setStartDate(""); setEndDate(""); }}
...
function HourlyView({ outlets }: { outlets: Outlet[] }) {
...
// ── Main component ─────────────────────────────────────────────────────────────
export default function TenantOrderHistoryTab() {
  const [view, setView] = useState<"orders" | "hourly">("orders");
  const [outlets, setOutlets] = useState<Outlet[]>([]);

  useEffect(() => {
    fetchOutlets().then((r) => setOutlets(r.data || [])).catch(() => {});
  }, []);
  ...
  return (
    <div>
      ...
      {view === "orders" ? <OrdersView outlets={outlets} /> : <HourlyView outlets={outlets} />}
    </div>
  );
}
